...
export function NetWorthChart() {
  ...
  const chartData = {
    labels: sortedEntries.map(entry => format(new Date(entry.date), 'MMM dd, yyyy')),
  ...
  const options: ChartOptions<"line" | "bar"> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: chartType === "stacked" ? 'Assets vs Liabilities' : 'Net Worth Growth',
        color: '#6b7280',
        font: {
          size: isMobile ? 13 : 15,
          weight: 500,
          ...
          <Bar 
            ...
            data={{
              ...
              datasets: chartData.datasets.map(dataset => ({
                ...dataset,
                backgroundColor: dataset.backgroundColor instanceof Function ? 
                  'rgba(124, 58, 237, 0.5)' : 
